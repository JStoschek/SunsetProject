...
/// Supplies each pool worker with its own elevation/water abstractions for a
...
struct StripSources {
    ...
    virtual WaterQuery&      water_for_worker(int worker) = 0;
};
...
/// get_elevation / is_water paths carry no locks or shared mutation.
