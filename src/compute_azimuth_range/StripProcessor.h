...
// Computes the Azimuth Range result for one Latitude Strip.  Owns the whole
// per-strip sequence behind a single interface:
//
...
// sweep over them, masks water, and lets the frozen caches die with the call.
// The shared loaders and thread pool are reused across every strip.
//
// What stays outside the seam: all GeoTIFF I/O (band layout, the south→north
// vertical flip, RasterIO writes) remains in the caller, so this module is pure
// terrain orchestration and can be tested without writing a file.
...
public:
    ...
    // Freeze → sweep → mask for the latitude band [strip_min_lat, strip_max_lat]
    // across the configured longitude extent.  Returns the masked per-pixel
    // min/max azimuth buffers (row 0 = south).
