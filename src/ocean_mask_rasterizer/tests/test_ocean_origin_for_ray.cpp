...
int main() {
    ...
    // The stub returns the input unchanged (-122.7°W), so this fails.
    {
        OceanMaskRasterizer omr(GSHHG_FULL_PATH);
        auto [origin_lat, origin_lon] = omr.ocean_origin_for_ray(az, lat, lon);
        assert(origin_lon < -123.5 && "origin must be in open Pacific, not near shore");
    ...
    // Stub distance = 0, so this fails.
    {
        OceanMaskRasterizer omr(GSHHG_FULL_PATH);
        auto [origin_lat, origin_lon] = omr.ocean_origin_for_ray(az, lat, lon);
        const double dist = haversine_km(lat, lon, origin_lat, origin_lon);
    ...
    // The 200 km offset into open Pacific should land on water.
    {
        OceanMaskRasterizer omr(GSHHG_FULL_PATH);
        auto [origin_lat, origin_lon] = omr.ocean_origin_for_ray(az, lat, lon);
        assert(omr.is_water(origin_lat, origin_lon) && "origin must be in water");
    ...
    // the near-shore input to the origin should be within ±10° of 270°.
    {
        OceanMaskRasterizer omr(GSHHG_FULL_PATH);
        auto [origin_lat, origin_lon] = omr.ocean_origin_for_ray(az, lat, lon);
        const double b = bearing_deg(lat, lon, origin_lat, origin_lon);
